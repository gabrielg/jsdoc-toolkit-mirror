...
JsDoc.File = function(path) {
	this.path = path;
	this.name = JsDoc.Util.fileName(path);
	this.overview = new Symbol(this.name, [], "", "FILE", null, new JsDoc.Doclet(""));;
	this.symbols = [];
}
...
JsDoc.parse = function(srcFiles) {
	...
	for (var i = 0; i < srcFiles.length; i++) {
		...
		inform("\t"+tokens.length+" tokens found.");
		...
		parser.parse(ts);
		inform("\t"+parser.symbols.length+" symbols found.");
...
JsDoc.main = function() {
	...
	inform("Source files found:\n\t"+srcFiles.join("\n\t"));
	
	if (!JsDoc.opt.d) JsDoc.opt.d = "js_docs_out";
	inform("Creating output directory: "+JsDoc.opt.d);
	MakeDir(JsDoc.opt.d);
