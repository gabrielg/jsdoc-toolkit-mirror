		...
		if (ts.look().is("NAME") && ts.look(1).is("COLON")) {
			...
			if (ts.look(2).is("FUNCTION")) {
				...
				if (name) {
					...
					this.onFnBody(new TokenStream(body), [name]);
				}
			}
			// like foo: {...}
	...
	while (fs.look()) {
		if (fs.look().is("NAME") && fs.look(1).is("ASSIGN")) {
			...
			if (/^this\./.test(name)) {
				...
				// handle object literals like this.foo = { bar: function(){} }
				if (fs.look(2).is("LEFT_CURLY")) {
					var parent = scope[scope.length-1];
					fullname = name.replace("this.", "{"+parent+"}.");
					print("got obliteral property: "+fullname);
					var literal = fs.balance("LEFT_CURLY");
					this.onObLiteral(new TokenStream(literal), [fullname]);
				}
				...
				if (fs.look(2).is("FUNCTION")) {
					...
					scope.push("{"+scope.pop()+"}"); // parent is an instance
					var path = scope.join(".");
					...
					this.symbols.push(
						new Symbol(path+"."+name, params, body, type, null, doc)
					);
					
					if (body) {
						scope.push(name);
						this.onFnBody(new TokenStream(body), scope); // recursive
						scope.pop();
