/**
 * @projectOverview Find functions in tokenized JavaScript source code.
	...
	while (ts.next()) {
		if (ts.look().is("NAME") && ts.look(1).is("COLON")) {
			if (ts.look(2).is("FUNCTION")) {
				...
				if (name) {
					var paramTokens = ts.matching("LEFT_PAREN");
					var params = [];
					...
					var body = ts.matching("LEFT_CURLY");
					
					if (!(doc && /@ignore\b/i.test(doc))) {
						this.symbols.push(
				...
				var inner = ts.matching("LEFT_CURLY");
				...
				while (!ts.look().is("COMMA")) { // skip to end of RH value ignoring ({bla, bla})
					if (ts.look().is("LEFT_PAREN")) ts.matching("LEFT_PAREN");
					else if (ts.look().is("LEFT_CURLY")) ts.matching("LEFT_CURLY");
...
JsParse.prototype.findObLiteral = function(ts) { /*dbg*///print("findObLiteral  "+ts.look());
	if (ts.look().is("NAME") && ts.look(1).is("ASSIGN")) {
		...
		if (ts.look(2).is("LEFT_CURLY")) {
			var literal = ts.matching("LEFT_CURLY");
...
JsParse.prototype.findFunction = function(ts) { /*dbg*///print("findFunction "+ts.look());
	if (ts.look().is("FUNCTION")) {
		...
		// like var foo = function()
			...
			paramTokens = ts.matching("LEFT_PAREN");
			body = ts.matching("LEFT_CURLY");
			if (ts.look(1).is("LEFT_PAREN")) {
				type = SYM.OBJECT;
				ts.matching("LEFT_PAREN");
			}
		}
		...
		if (name) {
			if (type == SYM.FUNCTION) {
				if (!paramTokens.length) paramTokens = ts.matching("LEFT_PAREN");
				for (var i = 0; i < paramTokens.length; i++) {
					if (paramTokens[i].is("NAME"))
						params.push(paramTokens[i].data);
				}
			}
			if (!body) body = ts.matching("LEFT_CURLY");
				...
				if (fs.look().is("FUNCTION")) {
					if (fs.look(-1).is("ASSIGN") && fs.look(-2).is("NAME")) {
						...
						if (/^this\./.test(method)) {
							...
							method = method.replace("this.", name+"/");
							var mParamTokens = fs.matching("LEFT_PAREN");
							var mParams = [];
							...
							var mBody = fs.matching("LEFT_CURLY");
							
							if (!(doc && /@ignore\b/i.test(doc))) {
								this.symbols.push(
									new Symbol(method, mParams, mBody, type, null, mDoc)
								);
							}
						}
					}
